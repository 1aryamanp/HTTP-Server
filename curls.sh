...
SESSION_COOKIE=$(curl -i -v -X POST -H "username: wronguser" -H "password: 3TQI8TB39DFIMI6" "http://${SERVER_IP}:${SERVER_PORT}/" | grep -i 'Set-Cookie' | cut -d ' ' -f 2 | cut -d '=' -f 2)

SESSION_COOKIE=$(curl -i -v -X POST -H "username: Richard" -H "password: wrongpassword" "http://${SERVER_IP}:${SERVER_PORT}/" | grep -i 'Set-Cookie' | cut -d ' ' -f 2 | cut -d '=' -f 2)
